...
function UserScore({ userscore, setuserscores, token, id, collections }) {
  //states
  const [isEditing, setIsEditing] = useState(false);
  const [text, setText] = useState(userscore.title);
  ...
  const makeCopy = async () => {
    ...
    const userscores = `https://api.flat.io/v2/users/${id}/scores`;
    fetch(userscores, {
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
