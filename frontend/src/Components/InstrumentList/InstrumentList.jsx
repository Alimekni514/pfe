...
const InstrumentList = ({titleobject,token,collection,setModalIsOpen1,setuserscores}) => {
  ...
  const groups = {
    brass: ['trumpet', 'trombone', 'horn','horninc','hornind','baritonehorn','tuba'],
  ...
  const handleSearch=(event)=> {
    ...
    const filteredGroup=Object.keys(groups).find((group)=> {
      return groups[group].some((instrument)=> filteredInstruments.includes(instrument))
    })
  ...
  const handletest=(e)=> {
    ...
    const instrumentList=Object.values(instruments).filter(({count})=>count>0).map(({group,name,count})=> {
  ...
  />
      ...
      <ul>
        {Object.keys(groups).map((group) => (
          <li key={group}>
            ...
            >
               <img src={groupsimage[group]} alt={group} style={{width:"30px"}} />
              {group}
      ...
      <div className='instrumentlistbody'>
                <div><span>{InstrumentsWithCount.length} </span> Of <span>{Object.keys(instruments).length}</span>Selected</div>
                <button className='createsc' onClick={handletest}>Create Score</button>
      </div>
    </div>
  );
};
